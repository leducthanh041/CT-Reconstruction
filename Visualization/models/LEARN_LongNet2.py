...
# --- Kế thừa DilatedAttention để lưu attention matrix ---
class DilatedAttentionWithAttn(DilatedAttention):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch_size, seq_len, _ = x.shape
        
        # Padding để chiều dài seq_len trở thành bội số của segment_size
        padding_len = -seq_len % self.segment_size
        x = F.pad(x, (0, 0, 0, padding_len))
        seq_len = seq_len + padding_len
        
        # Nếu sử dụng xpos, thêm positional encoding
        if self.use_xpos:
            x = self.xpos(x)

        # Chia nhỏ x thành các segment có kích thước segment_size
        x = x.view(batch_size, -1, self.segment_size, self.dim)
        
        # Dilation: chọn token có khoảng cách dilation_rate
        x = x[:, :, :: self.dilation_rate, :]

        # Chuẩn hóa q, k, v nếu cần
        if self.qk_norm:
            q, k, v = map(self.norm, (self.proj_q(x), self.proj_k(x), self.proj_v(x)))
        else:
            q, k, v = self.proj_q(x), self.proj_k(x), self.proj_v(x)

        # Reshape cho nhiều head
        heads = self.heads
        d_k = self.dim // heads
        q = q.view(batch_size, -1, self.segment_size // self.dilation_rate, heads, d_k).permute(0, 3, 1, 2, 4)  # B, heads, N, L, d_k
        k = k.view(batch_size, -1, self.segment_size // self.dilation_rate, heads, d_k).permute(0, 3, 1, 2, 4)
        v = v.view(batch_size, -1, self.segment_size // self.dilation_rate, heads, d_k).permute(0, 3, 1, 2, 4)

        # Tính attention scores (batch, heads, N, L, L)
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(d_k)

        # Áp dụng softmax để chuẩn hóa scores thành ma trận attention weights
        ...
        # Trả về kết quả dạng gốc (batch, N, L, dim)
        attn_output = attn_output.permute(0, 2, 3, 1, 4).contiguous()
        attn_output = attn_output.view(batch_size, -1, self.segment_size // self.dilation_rate, self.dim)

        # Dạng kết quả cuối cùng (batch, N * L, dim)
...
class LongNetAttentionBlock(nn.Module):
    ...
    def __init__(self, window_size=2, patch_channels=48, image_size=256):
        ...
        self.longnet_attention = DilatedAttentionWithAttn(dim=self.token_dim, heads=4, 
                                                        dilation_rate=1, segment_size=1024, 
                                                        dropout=0.1, qk_norm=True,
...
class LEARN_LongNet_pl(pl.LightningModule):
    def __init__(self, n_iterations,num_view, num_detectors):
        super(LEARN_LongNet_pl, self).__init__()
    ...
    def forward(self, x_t, y):
        ...
        for i in range(self.num_iter):
            ...
            outputs.append(x_t.detach().cpu().squeeze().numpy())
        return x_t, outputs  # Trả về cả ảnh cuối cùng và list các ảnh qua từng bước
