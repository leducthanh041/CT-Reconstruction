...
class NystromformerAttentionBlock(nn.Module):
    ...
    def __init__(self, window_size=2, patch_channels=48, image_size=256):
        ...
        self.nystromformer_attention=NystromAttention(
                                    dim = self.token_dim,
    ...
    def forward(self, x):
        ...
        # # Tạo position_ids với shape (batch, seq_len)
        # position_ids = torch.arange(tokens.shape[1], dtype=torch.long, device=tokens.device).unsqueeze(0).expand(batch_size, -1)
       
        # Áp dụng lớp attention của Nystromformer
        attention_output = self.nystromformer_attention(tokens)
...
class RegularizationBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.Nystromformer_attn(x)  # Attention: token hóa -> attention -> expand lại -> (batch, 48, 256, 256)
