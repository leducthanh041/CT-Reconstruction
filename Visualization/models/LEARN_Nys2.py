...
class NystromAttentionWithLogging(NystromAttention):
    def __init__(self, *args, **kwargs):
        super(NystromAttentionWithLogging, self).__init__(*args, **kwargs)
        self.attn1 = None
        self.attn2_inv = None
        self.attn3 = None
        self.attn = None
    ...
    def forward(self, x, mask=None, return_attn=False, return_attn_matrices=False):
        ...
        out, attn = super(NystromAttentionWithLogging, self).forward(x, mask, return_attn=True, return_attn_matrices=True)
        
        self.attn = attn
        return out, self.attn  # Trả về kết quả và ma trận attention nếu không cần ma trận riêng biệt


# ----------------------------
# Lớp NystromformerAttentionBlock: áp dụng attention lên output của Conv (tokenization, attention, expand lại)
# ----------------------------
class NystromformerAttentionBlock(nn.Module):
    """
    ...
    def __init__(self, window_size=2, patch_channels=48, image_size=256):
        ...
        # Khởi tạo NystromformerSelfAttention cho tất cả các lớp attention
        self.nystromformer_attention = NystromAttentionWithLogging(
            dim=self.token_dim,
            dim_head=patch_channels,
            heads=4,
            num_landmarks=image_size,    # number of landmarks
            pinv_iterations=6,           # number of Moore-Penrose iterations for approximating pinverse
            residual=True,               # whether to do an extra residual with the value
    ...
    def forward(self, x):
        ...
        # Áp dụng Nystromformer attention và lưu ma trận attention
        attention_output, attn = self.nystromformer_attention(tokens, return_attn_matrices=True)
        ...
        return reconstructed, attn  # Trả về cả ảnh tái tạo và attention map
...
class RegularizationBlock(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, kernel_size=5):
        ...
        NEW
        '''
        # Lớp Nystromformer attention block được áp dụng sau Conv1
        self.Nystromformer_attn = NystromformerAttentionBlock(window_size=2, patch_channels=48, image_size=256)
        '''
        NEW
        '''
    ...
    def forward(self, x):
        ...
        x, _ = self.Nystromformer_attn(x)  # Attention: token hóa -> attention -> expand lại -> (batch, 48, 256, 256)
...
class LEARN_Nys_pl(pl.LightningModule):
    def __init__(self, n_iterations,num_view, num_detectors):
        super(LEARN_Nys_pl, self).__init__()
    ...
    def forward(self, x_t, y):
       outputs = []
       attn_maps = []  # Lưu tuple của 3 ma trận attention
       for i in range(self.num_iter):
           x_t = x_t - self.gradient_list[i](x_t, y, self.forward_module, self.backward_module)
           outputs.append(x_t.squeeze(0).squeeze(0).detach().cpu().numpy())

           # Lấy tuple (attn1, attn2_inv, attn3) từ Nystromformer
           attn_tuple = self.gradient_list[i].regularitation_term.Nystromformer_attn.nystromformer_attention.attn
           attn_maps.append(attn_tuple)  # Lưu tuple ma trận

       return x_t, outputs, attn_maps  # Trả về cả các ma trận và ảnh tái tạo
