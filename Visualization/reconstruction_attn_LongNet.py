...
for i in range(1):
    ...
    sino_longnet = to_batch_tensor(sino_raw, device)

    # Tính toán đầu ra từ mô hình
    with torch.no_grad():
        y_hat_longnet, _ = model_longnet(fbp_u_longnet, sino_longnet)

    # Tính toán PSNR và SSIM cho ảnh hiện tại
    ssim_value = ssim_metric(y_hat_longnet, phantom).item()
    psnr_value = psnr_metric(y_hat_longnet, phantom).item()
...
model_longnet.to(device).eval()
with torch.no_grad():
    y_hat_longnet, outputs = model_longnet(fbp_u_longnet, sino_longnet)

# Tính PSNR/SSIM cho ảnh tốt nhất sau khi inference
ssim_best = ssim_metric(y_hat_longnet, phantom).item()
psnr_best = psnr_metric(y_hat_longnet, phantom).item()

print(f"After inference, PSNR: {psnr_best:.2f}, SSIM: {ssim_best:.4f}")

attn_maps = []
for i in range(model_longnet.num_iter):
    attn = model_longnet.gradient_list[i].regularitation_term.longnet_attn.longnet_attention.attn_weights
