...
class LongNetAttentionBlock(nn.Module):
    ...
    def __init__(self, window_size=2, patch_channels=48, image_size=256):
        ...
        self.longnet_attention = DilatedAttention(dim=self.token_dim, heads=4, dilation_rate=1, segment_size=1024, dropout=0.1, qk_norm=True)
...
class RegularizationBlock(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, kernel_size=5):
        ...
        self.longnet_attn = LongNetAttentionBlock(window_size=16, patch_channels=48, image_size=256)
        '''
        NEW
        '''
