...
# ----------------------------
# L?p LongformerAttentionBlock: �p d?ng attention l�n output c?a Conv (tokenization, attention, expand l?i)
# ----------------------------
class LongformerAttentionBlock(nn.Module):
    ...
    def forward(self, x, attention_mask=None):
        ...
        if attention_mask is None:
            # Longformer d�ng 0 = local, -10000 = no-attn, +10000 = global
            # ? d�y ta kh�ng mu?n no-attn, ch? local + global
        ...
        # 3) Ch?n evenly 50 global indices tr�n m?i sequence
        #    v� d?: linspace t? 0 t?i N-1 th�nh num_global bu?c
        ...
        # d�nh d?u cho m?i m?u trong batch
        ...
        #    truy?n th�m is_index_global_attn v� is_global_attn
        ...
        # 5) Reshape ngu?c l?i (B, 48, 256, 256)
...
class RegularizationBlock(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, kernel_size=5, patch_channels=48):
        ...
        padding_value = kernel_size // 2  # V?i kernel_size=5 th� padding = 2
        ...
        NEW
        '''
        # L?p Longformer attention block du?c �p d?ng sau Conv1
        self.longnet_attn = LongformerAttentionBlock(window_size=8, patch_channels=patch_channels, image_size=256)
        '''
        NEW
        '''
    ...
    def forward(self, x):
        ...
        x = self.longnet_attn(x)  # Attention: token h�a -> attention -> expand l?i -> (batch, 48, 256, 256)
...
class LEARN_pl(pl.LightningModule):
    ...
    def forward(self,x_t,y):
        # x_t l� h�nh ?nh ban d?u (ho?c h�nh ?nh du?c c?p nh?t)
        x_t = x_t
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        x_t = fbp_u # H�nh ?nh kh?i t?o ban d?u
        y = sino_noisy
        initial = torch.rand(y.shape[0],1,256, 256).cuda() # N?u c?n d�ng l�m kh?i t?o
