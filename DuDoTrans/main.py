...
class Trainer(object):
  def __init__(self, setting, ST, learning_rate=0.0001, is_restart=True, max_epoch=50, is_cuda=True, num_view=32, poission_level=5e5):
    ...
    self.save_results_path = "./visualization_results_deep-aapm/"
    os.makedirs(self.save_results_path, exist_ok=True)
    self.setting = setting

    # Data Flow Pipiline
    print('Reading CT slices Beginning')
    self.train_dataset=CTSlice_Provider('/home/thanhld/CT_Reconstruction/split/', setting= self.setting, poission_level=self.poission_level, gaussian_level=self.gaussian_level, num_view=self.num_view, transform=transform)
    ...
    if not os.path.exists('./results/models'+ '_' + ST):
      os.mkdir('./results/models'+ '_' + ST)
    if not os.path.exists('./results/visualization'+ '_' + ST):
      os.mkdir('./results/visualization'+ '_' + ST)
    if self.is_restart:
      shutil.rmtree('./results/visualization'+ '_' + ST)
      os.mkdir('./results/visualization'+ '_' + ST)
      self.epoch=0
      self.global_iter=0
      ...
      try:
        state=torch.load('./results/models'+ '_' + ST +'/epoch_24iter1999.pth.tar')
    ...
    for e in range(self.epoch, self.max_epoch):
      ...
      with tqdm(desc='Epoch %d - train' % (e+1), unit='it', total=len(self.train_loader)) as pbar:
        ...
        if e%2==0:
          ...
          sitk.WriteImage(to_save_image_, './results/visualization'+ '_' + ST+'/pred_epoch_{}_iter_{}_{:.2f}_{:.2f}.nii.gz'.format(e, num_iter, curr_psnr, curr_ssim))
        ...
        #   to_save_image = reconstructed_image[0, 0, ...].detach().cpu().numpy()
        #   to_save_image_ = sitk.GetImageFromArray(to_save_image)
        #   sitk.WriteImage(to_save_image_, './results/visualization/pred_epoch_{}_avg_psnr_{:.2f}_ssim_{:.4f}.nii.gz'.format(e, avg_psnr, avg_ssim))

      # self.inference()

  def save_checkpoint(self, num_epoch, state, num_iter):
    base_path='./results/models' + '_' + ST
  ...
  def inference(self):
    # Data Flow Pipiline
    print('Reading CT slices Beginning')
    self.test_dataset=CTSlice_Provider('/home/thanhld/CT_Reconstruction/split/', setting= self.setting, poission_level=self.poission_level, gaussian_level=self.gaussian_level, num_view=self.num_view, transform=transform)
    self.test_loader=DataLoader(dataset=self.test_dataset, batch_size=1, shuffle=False)

    state=torch.load('./results/models'+ '_' + ST +'/epoch_48iter1919.pth.tar')
    self.reconstructor_func.load_state_dict(state['reconstructor_state'])
