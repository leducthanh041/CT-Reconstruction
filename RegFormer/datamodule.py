...
class CTDataModule(pl.LightningDataModule):
    def __init__(self,data_dir, batch_size, setting: str, poission_level, num_view=64,input_size = 256, num_select = -1):
        super().__init__()
    ...
    def setup(self, stage):
        ...
        if stage == "fit" or stage is None:
            ...
            # self.ct_valid = CTSlice_Provider(self.data_dir, setting = self.setting, poission_level = self.poission_level,test=True, num_view=self.num_view, input_size=self.input_size, transform=self.transform,num_select=self.num__select)
            # print('Leng of CT VALID:',len(self.ct_train))
        if stage == "test" or stage is None:
            self.ct_test = CTSlice_Provider(self.data_dir, setting = self.setting, poission_level = self.poission_level, num_view=self.num_view, input_size=self.input_size, transform=self.transform,test=True,num_select=self.num__select)
