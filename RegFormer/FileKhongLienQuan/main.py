...
class net():
    def __init__(self):
        self.model = model.regformer(opt.n_block, views=64, dets=368, width=256, height=256, 
            dImg=0.006641*2, dDet=0.012858*2, dAng=0.006134*16, s2r=5.95, d2r=4.906, binshift=0)
        self.loss = nn.MSELoss()
        self.path = opt.model_save_path
        self.train_data = DataLoader(trainset_loader("/data/uittogether/LuuTru/Thanhld/Sparse-view-CT-reconstruction-main/AAPM_dataset", '64'),
            batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)
        self.vali_data = DataLoader(valiset_loader("/data/uittogether/LuuTru/Thanhld/Sparse-view-CT-reconstruction-main/AAPM_dataset", '64'),
            batch_size=opt.batch_size*4, shuffle=False, num_workers=opt.n_cpu)   
        self.test_data = DataLoader(testset_loader("/data/uittogether/LuuTru/Thanhld/Sparse-view-CT-reconstruction-main/AAPM_dataset", '64'),
            batch_size=opt.batch_size*4, shuffle=False, num_workers=opt.n_cpu)             
        self.start = 0
